...
<body>
  ...
  <div id="wrap_global">
    ...
    <form action="daily_modyfiAction" method="post" name="write_daily" enctype="multipart/form-data">
    <input type="hidden" name="category" value="일상">
    <input type="hidden" name="no" value="<%=daily_dto.getNo()%>">
    <input type="hidden" name="id" value="<%=sessionID%>">
      <table class="writeForm">
        <tr>
          <td class="writeForm">대표이미지</td>
          <td><input type="file" name="filename"  id="filename" value="<%=daily_dto.getFilename()%>"></td>
        </tr>
        <tr>
          <td class="writeForm">제목</td>
          <td><input type="text" name="title" size="50" value="<%=daily_dto.getTitle()%>"></td>
        </tr>
        <tr>
      ...
      <textarea name="content" id="" cols="120" rows="40" style="resize: none;" ><%=daily_dto.getContent() %></textarea>
      <script>CKEDITOR.replace('content',{height : '800px',width:'1000px',filebrowserUploadUrl:'/mine/imageUpload.do'});</script>
      <input type="button" value="글 수정" class="daily_button" onclick="checkValue()"style="margin-left: 800px;"> 
    </form>
    <a href="/daily"><button class="daily_button" style="position: absolute; bottom: 0px;margin-left: 900px" >글 목록</button></a>    
  	</div>
