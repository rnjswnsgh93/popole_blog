...
.view_image{
	...
	background-size:100% 500px;
 	background-repeat: no-repeat;
      z-index: -1;
      opacity: 0.8;
     background-image:url('upload/<%=taste_dto.getFilename()%>');
...
<script type="text/javascript">
	function goCheck(value){
		if(value =="1"){
			location.href="taste?pagenum=1&pagelast=5";}
		
		if(value =="2"){
			location.href ="write_taste";	
		}
	}	
	function button_modyfi(data){
		location.href ="taste_modyfi?no="+data;
		}
	function button_delete(data){
			location.href ="taste_delete?no="+data;	
		}
...
<body>
  ...
  <div class="wrap_global">
    ...
    <br><br>
    <p><a href="/">home</a>><a href="#">맛집</a>>맛집 글</p>
    <br><br>
	</div>
    <div class="view_body">
    <div class="view_image"><br><br>
    <div class="top_title">
    <h2><%=taste_dto.getTitle()%></h2>
     <%if(sessionID != null && sessionID.equals(taste_dto.getId())){ %>
   		<button onclick="button_modyfi(${taste_dto.no})" class="taste_button">수정</button>
   		<button onclick="button_delete(${taste_dto.no})" class="taste_button">삭제</button>
    <%} %>
     <br><br><br><br><br><p><%=taste_dto.getReg() %></p>
    <p>작성자 :<u><%=taste_dto.getId() %></u></p>
    </div>
	</div>
	<div class="wrap_global">
    <hr><br><br>
     <div style="width: 1000px; min-height: 1000px;  border-top: 0.5px solid #e7e5e2 ; border-bottom: 0.5px solid #e7e5e2 ;"><p><%=taste_dto.getContent() %></p></div>
    <div style="margin-left: 950px;">
      <span><input type="button" value="글 목록" onclick="goCheck(1)" class="taste_button"></span>
      <span><input type="button" value="글쓰기" onclick="goCheck(2)" class="taste_button"></span>
    </div>
  
  

    <br><br>
    <div style="border-top: 3px solid #e4e8ff;border-bottom: 3px solid #e4e8ff; width: 1100px; background-color: #fff">
      <h4>최신 맛집글</h4><br>
      <c:forEach var="dto" items="${ taste_list }" end="3">
     <div><i><a href="taste_view?no=${dto.no}&pagenum=1&pagelast=5" style="font-size: 20px; color:black ">${dto.title}</a></i>&nbsp;&nbsp;<div style="float: right;">[${dto.reg}]</div></div>
      </c:forEach>
    </div>
    ...
    <div style="width: 1080px;">
    	...
    	<input type="hidden" name="viewno" value="<%=taste_dto.getNo()%>">
